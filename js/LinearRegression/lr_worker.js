...
function lcgRand(seed) {
  ...
  function splitData(rows, testRatio, seed, splitMethod) {
    if (splitMethod === 'linear') {
      return linearSplit(rows, testRatio);
    }
    return trainTestSplit(rows, testRatio, seed); // default: random LCG (perilaku lama)
  }
  // ── Konversi float ────────────────────────────────────────────
  ...
  self.onmessage = function (e) {
    ...
    try {
      const { rawRows, numericCols, featureCols, targetCol, testRatio, seed, reg, lambda, splitMethod } = e.data.payload;
      ...
      // Step 2 — Split
      const splitLabel = splitMethod === 'linear' ? 'Linear (sistematis)' : 'Random (LCG)';
      progress(6, `Train/test split (${splitLabel})...`);
      const { train, test } = splitData(numRows, testRatio, seed, splitMethod);
