...
function processLR() {
  ...
  const splitMethod = (document.querySelector('input[name="split-method"]:checked') || {}).value || 'random';
...
function renderResultPage(model, trainRows, testRows, testMetrics) {
  ...
  rc.innerHTML = `
    ...
    <div class="page-footer">
      ...
      <button class="btn btn-green" onclick="exportPlainText(LR_STATE.model, LR_STATE.trainRows, LR_STATE.testRows)">
        &#8659; Export Excel (Plain Text)
      </button>
      <button class="btn btn-green" onclick="exportFormula(LR_STATE.model, LR_STATE.trainRows, LR_STATE.testRows)">
